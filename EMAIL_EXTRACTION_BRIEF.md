...
The current enrichment worker uses a ten-review cap. Owner/contact and email methodology were unchanged by this adjustment. See current status before any enrichment rerun; the missing-source save defect remains pending.
...
# Historical: Website Email Discovery Phase 7

> **Archived implementation snapshot, rechecked 2026-06-29.** `emailsDiscovered`, `emailDiscoveryCategory`, and the later `emailCandidates` evidence contract already exist in both live schemas. The current enrichment worker discovers ranked candidates and the current Emailable cleaner verifies candidate sets before selecting a primary email. The migration/backfill instructions and old coverage counts below are historical; do not run them from this file.
